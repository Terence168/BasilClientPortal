<template>
  <div class="container">
    <div class="ticket-grid text-body2">
      <div class="row grid-header text-weight-medium text-center items-center">
        ...
        <div class="col-4">
          <div class="row">
            ...
            <div class="col-4 box" @click="sort('serialNumber')">
              <img
                style="width: 5%"
                v-bind:src="getIconSerialNumberPath"
              />Serial Number
            </div>
            <div class="col-5" @click="sort('model')">
              <img style="width: 4%" v-bind:src="getIconModelPath" />Model
              Number Short
            </div>
        ...
        >
          <div class="col-4">
            <div class="row items-center">
              ...
              <div class="col-5" v-show="serialData.valid">
                {{ serialData.model }}
              </div>
              <div class="col-2" v-show="serialData.valid">
                {{ serialData.version }}
              </div>
            </div>
          </div>
          <div class="row items-center" v-show="!serialData.valid">
            <div class="col-12 text-bold">
              Your SN was not found. Please remove this record and contact the
              RMA Support.
            </div>
          </div>
          ...
          <div class="col-4" v-show="serialData.valid">
            <div class="row items-center">
              ...
              <div class="col-2" v-else>
                {{
                  serialData.cosmetic === true ? serialData.cosmeticPrice : 0
                }}
              </div>
            </div>
          </div>
      ...
      <div
        v-if="getSerials === undefined || getSerials.length == 0"
        class="row grid-row text-center items-center"
      >
        <div class="col-12 text-center">No Serial Number Input</div>
      </div>
      <div class="row grid-row justify-end bg-white">
        <div class="text-h6">Total Estimated Cost: $ {{ totalInvoice }}</div>
      </div>
    </div>
    ...
    addPopupBtns(event) {
      ...
      if (this.removeUnitWidth === null) {
        ...
        this.positionStyle["top"] =
          this.mouseY - this.removeUnitWidth / 2 + "px";
        this.positionStyle["left"] =
          this.mouseX - this.removeUnitWidth - this.spaceInBtnGroup / 2 + "px";
      }
    },
